  ...
  <div class="module">
    ...
    <el-row>
      <el-col el-col :span=24 style="text-align: center;">
        ...
        <el-button type="warning" size="small" icon="el-icon-download" :disabled="this.exportLoading"  v-permission="['/cityManage/cityQuery/export']"  @click="exportClick" >导出</el-button> <span v-loading="this.exportLoading" element-loading-spinner="el-icon-loading"></span>
      </el-col>
...
<script>
  ...
  export default {
      ...
      methods: {
        ...
        exportClick() {
          //此代码实现向后台异步请求数据
          cityApi.queryCity(this.query).then(response => {
        ...
        export2Excel() {
          import('@/vendor/Export2Excel').then(excel=>{
            const tHeader =
              ['国际代码', '城市', '省/市','拼音全称','状态', '修改时间','修改人'];
            const filterVal =
              ['code', 'cnName','provinceName','enName','status','modifyTime', 'modifyEmpName'];
            const list = this.exportList;
            const data = this.formatJson(filterVal, list);
            excel.export_json_to_excel({header: tHeader, data, filename: '城市导出' + parseTime(new Date())})
          })
