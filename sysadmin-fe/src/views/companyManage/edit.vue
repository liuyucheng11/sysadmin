<template>
  <div>
    <div class="actions">
      <el-button type="primary" @click="save" v-if="showButton">保存</el-button>
      <el-button type="primary" @click="cancel">取消</el-button>
    </div>

    <div class="form-container">
      <el-form :model="company" :rules="checkRules" ref="form" :disabled="onlyShow">
        <div class="base-info">
          ...
          <el-row class="row">
            <el-col :span="8">
              <el-form-item label="类型:" prop="companyType">
                <div class="form-item">
                  <el-select v-model="company.companyType" clearable="true">
        ...
        <div class="annex-information">
          ...
          营业执照附件:
          <div>
            <div class="license-load">
              已上传附件:
              <license v-for="license in licenses" :license="license" :key="license.id" :onlyShow="onlyShow"
          ...
          <el-row class="row">
            <el-col :span="8">
              <el-form-item label="新建人">
                <div class="form-item">
                  <el-input v-model="company.createEmpName" readonly="true" style="width: 300px"></el-input>
                </div>
              </el-form-item>
            </el-col>
            <el-col :span="8">
          ...
          <el-row class="row">
            <el-col :span="8">
              <el-form-item label="修改人:">
                <div class="form-item">
                  <el-input v-model="company.modifyEmpName" readonly="true" style="width: 300px"></el-input>
                </div>
              </el-form-item>
            </el-col>
            <el-col :span="8">
...
<script>
  import {deleteLicense} from "../../api/company";
  import {modifyCompany} from "../../api/company.js";
  ...
  export default {
    ...
    data() {
      ...
      return {
        ...
        statusOptions:[{label:'有效',value:0},{label:'无效',value:1}],
        uploadUrl: 'aaa',
        iscommit: true,
        fileList: [],
        uploadFiles: [],
    ...
    mounted() {
      ...
      this.onlyShow = this.$route.params.onlyShow;
    ...
    computed: {
      maxUpload() {
        return 20 - this.licenses.length;
      },
    ...
    methods: {
      ...
      save() {
        ...
        if (isValid) {
          ...
          if (this.iscommit) {
            ...
            modifyCompany(this.formData).then((re) => {
              this.formData = new FormData();
              this.$message.success("保存成功!");
              this.$refs['upload'].clearFiles();
              setTimeout(this.getCompanyInfo, 200);
      ...
      getCompanyInfo() {
        ...
        param.append('id', id);
        getCompanyById(param).then(re => {
          this_.company = re.data.company;
          this_.licenses = re.data.licenses;
        }).catch(re => {
        })
      },
      //删除执照
      deleteLicense(id) {
        this.$confirm("是否确认删除此执照附件?", '温馨提示', {
      ...
      handleExceed() {
        this.$message.error("超出最大文件上传个数(20)")
      }
    }
